    ...
    public class GenerateBridges
    {
        ...
        private static TerrainDef GenerateBridgeDef(ThingDef material, bool deep, string alternateTexture = "")
        {
            var currentBridgeType = new TerrainDef
            {
                ...
                {
                    RimWorld.TerrainAffordanceDefOf.Light, RimWorld.TerrainAffordanceDefOf.Medium,
            ...
            if (material.stuffProps.categories.Contains(StuffCategoryDefOf.Metallic))
            {
                ...
                if (deep)
                {
                    if (material.defName == "Steel")
                    {
                        ...
                        currentBridgeType.costList = new List<ThingDefCountClass>
                            {new ThingDefCountClass {thingDef = ThingDefOf.Steel, count = GetCustomCost(20)}};
                    }
                    else
                    {
                        var baseCost = 15;
                        ...
                        currentBridgeType.costList = new List<ThingDefCountClass>
                        {
                            new ThingDefCountClass {thingDef = ThingDefOf.Steel, count = GetCustomCost(5)},
                ...
                else
                {
                    if (material.defName == "Steel")
                    {
                        ...
                        currentBridgeType.color = DefDatabase<TerrainDef>.GetNamedSilentFail("MetalTile").color;
                        currentBridgeType.costList = new List<ThingDefCountClass>
                            {new ThingDefCountClass {thingDef = ThingDefOf.Steel, count = GetCustomCost(12)}};
                    }
                    else
                    {
                        var baseCost = 9;
                        ...
                        currentBridgeType.costList = new List<ThingDefCountClass>
                        {
                            new ThingDefCountClass {thingDef = ThingDefOf.Steel, count = GetCustomCost(3)},
                            new ThingDefCountClass {thingDef = material, count = GetCustomCost(baseCost)}
                        };
                    }
                }
